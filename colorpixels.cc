...
bool decode_avif(std::string_view filename, unsigned char *&pixels, int &width,
                 int &height)
{
...
bool decode_webp(std::string_view filename, unsigned char *&pixels, int &width,
                 int &height)
{
  std::ifstream in(std::string(filename), std::ios::binary | std::ios::ate);
  if (!in)
...
int main(int argc, char *argv[])
{
  ...
  app.add_option("-c,--chroma", chroma_threshold,
                 "Set chroma threshold for gray detection")
      ->check(CLI::PositiveNumber);
  app.add_flag("-s,--sepia", accept_sepia, "Use wider chroma threshold (~13)");
  app.add_flag("-p,--print-filename", print_filename,
               "Print filename before ratio");
